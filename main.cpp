...
--3 way comparison <=> for determining strong / weak / partial order in item/value equivalents
--C++20 Ranges capabilities
--Modules
--std::algorithm utilities.
...
class jEntity {
    public:
	    jEntity(){ std::cout << "Created jEntity" << std::endl;}
		~jEntity(){ std::cout << "Destroyed jEntity" << std::endl;}
		void Print() {}
};
...
int main()
{
	...
	cout << "________________________________________________________________\n";
	cout << "Testing object lifetimes with unique ptr and move logic.";
	cout << "________________________________________________________________\n";
	
	//Create a unique_ptr, a stack allocated object that CANNOT be copied. Memory of that allocated heap object will be freed upon used.
	std::unique_ptr<jEntity> entity = std::make_unique<jEntity>(); 
	std::unique_ptr<jEntity> ptrChange = std::move(entity);
	if (entity == nullptr) {
		std::cout << "entity ptr became empty with the std::move. It is freed." << std::endl;
	}
	ptrChange->Print();
